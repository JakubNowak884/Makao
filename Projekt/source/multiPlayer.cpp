...
#include "windows.h"
multiPlayer::multiPlayer(multiPlayerSettings* _prev)
{
	prev = _prev;
...
bool multiPlayer::loadDataFromBuffer(bool onlyDeck)
{
	...
	while (buffer[i] != 'e')
	{
		deck.push_back(std::make_shared<card>(figureNumber(buffer[i]), suitNumber(buffer[i + 1]), font, textureSuit.get(), 120, 180, 400, 400, sf::Color::White));
		i += 2;
	}
	...
	if (!onlyDeck)
	{
		if (host)
		{
			...
			if (ID < 0 && ID > 3)
				...
				infoPlayer[size_t(ID - 2)].setString(std::string(&buffer[202], buffer[201]) + std::to_string(int(buffer[9])));
		}
		else
		{
			int k = 1;
...
void multiPlayer::initializeInfo()
{
	...
	case 2:
		...
		cardback.push_back(std::make_unique<object>(120, 180, 400, 100, sf::Color::White));
	...
	case 3:
		...
		cardback.push_back(std::make_unique<object>(120, 180, 70, 400, sf::Color::White));
		cardback[0]->getShape().setTexture(textureSuit.get());
		cardback[0]->getShape().setTextureRect(sf::IntRect(296, 920, 145, 230));
		cardback.push_back(std::make_unique<object>(120, 180, 400, 100, sf::Color::White));
	...
	case 4:
		...
		cardback[1]->getShape().setTexture(textureSuit.get());
		cardback[1]->getShape().setTextureRect(sf::IntRect(296, 920, 145, 230));
		cardback.push_back(std::make_unique<object>(120, 180, 640, 400, sf::Color::White));
		...
		if (wonCounter == amountOfPlayers - 1)
		{
			if (host)
			{
				...
				for (int i = 0; i < amountOfPlayers - 1; i++)
				{
					...
					std::cout << int(temp[temp.length()]);
					if (int(temp[temp.length()]) == 0)
						temp[temp.length()] = amountOfPlayers;
					std::cout << int(temp[temp.length()]);
					results[int(temp[temp.length()])] = temp2;
				}
				std::cout << player->getWon();
				if (player->getWon() == 0)
